...
export default function SortableTaskRow({ projectId, task }: SortableTaskRowProps) {
  ...
  return (
    <div 
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-4 bg-white dark:bg-gray-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md transition-all group ${isDragging ? 'opacity-50 shadow-lg' : ''}`}
    >
      {/* ドラッグハンドル */}
      <div
        {...attributes}
        {...listeners}
        className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex-shrink-0"
        title="ドラッグして並び替え"
      >
